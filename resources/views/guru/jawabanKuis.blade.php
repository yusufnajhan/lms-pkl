...
<div class="mb-4">
    <nav class="flex mb-5" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
          ...
          <li>
            <div class="flex items-center">
              <svg class="w-6 h-6 text-red-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
              <a href="{{ route('kuis.read', $kuis->idkuis) }}" class="ml-1 text-red-700 hover:text-primary-600 md:ml-2 dark:text-red-300 dark:hover:text-white">Penilaian Kuis</a>
            </div>
          </li>
          <li>
            <div class="flex items-center">
              <svg class="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
              <span class="ml-1 text-gray-400 md:ml-2 dark:text-gray-500" aria-current="page">Jawaban Kuis</span>
            </div>
          </li>
        </ol>
    </nav>
    ...
    <footer class="mt-5 flex items-center justify-between mb-2">
        <div class="flex items-center">
            <p class="text-sm text-gray-600 dark:text-gray-400"><time pubdate datetime="2022-02-08" title="February 8th, 2022">Dikumpulkan pada: {{ $jawaban->first()->tanggal_pengumpulan }}</time></p>
        </div>
    </footer>
    @endif
</div>
...
<div class="p-4 bg-white border border-gray-200 rounded-lg shadow-sm dark:border-gray-700 sm:p-6 dark:bg-gray-800">
  ...
  <form class="overflow-y-auto lg:max-h-[60rem] 2xl:max-h-fit">
      ...
      <article class="mb-5">
        <p class="text-sm font-bold text-gray-600 dark:text-gray-400">Soal {{ $index + 1 }}:</p>
      ...
      @foreach($jawaban as $index => $j)
        <article class="mb-5">
            <p class="text-sm font-bold text-gray-600 dark:text-gray-400">Jawaban soal {{ $index + 1 }}:</p>
            <p class="mt-3 mb-2 text-gray-900 dark:text-white">
            {{ $j->jawaban }}
            </p>
        </article>
