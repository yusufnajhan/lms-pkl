    ...
    <form action="{{ route('kelas.update', $idkelas) }}" method="POST">
        ...
        <div class="grid grid-cols-6 gap-6">
            ...
            <div class="col-span-6 sm:col-span-3">
                <label for="indeks_kelas"
                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Indeks Kelas</label>
                <select name="indeks_kelas" id="indeks_kelas"
                    class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                    <option value="" disabled selected>Pilih Indeks Kelas</option>
                    <option value="A"{{ $indeks_kelas == 'A' ? 'selected' : '' }}>A</option>
                    <option value="B"{{ $indeks_kelas == 'B' ? 'selected' : '' }}>B</option>
                    <option value="C"{{ $indeks_kelas == 'C' ? 'selected' : '' }}>C</option>
                    <option value="D"{{ $indeks_kelas == 'D' ? 'selected' : '' }}>D</option>
                    <option value="E"{{ $indeks_kelas == 'E' ? 'selected' : '' }}>E</option>
                </select>
            ...
            <div class="col-span-6 sm:col-span-3">
                ...
                <input type="date" name="tanggal_dibuat" id="tanggal_dibuat"
                    ...
                    value="{{ $tanggal_dibuat }}" wfd-id="id2" readonly>
            ...
            {{-- <div class="col-span-6 sm:col-span-3">
                ...
                <select name="idguru" id="idguru" 
                    class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"wfd-id="id2">
                    <option value="{{ auth()->user()->dataPribadi->idguru }}" selected>{{ auth()->user()->dataPribadi->idguru }}</option>
                </select>
            </div> --}}
