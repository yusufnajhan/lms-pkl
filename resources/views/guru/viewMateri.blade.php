    ...
    <nav class="flex mb-5" aria-label="Breadcrumb">
      ...
      <li>
        <div class="flex items-center">
          ...
          <span class="ml-1 text-gray-400 md:ml-2 dark:text-gray-500" aria-current="page">Materi Kelas</span>
        </div>
      </li>
      </ol>
  </nav>
...
<div class="p-4 bg-white border border-gray-200 rounded-lg shadow-sm dark:border-gray-700 sm:p-6 dark:bg-gray-800">
    <ul class="hidden text-sm font-medium text-center text-red-500 divide-x divide-red-200 rounded-lg sm:flex dark:divide-red-600 dark:text-red-400" id="fullWidthTab" data-tabs-toggle="#fullWidthTabContent" role="tablist">
      <li class="w-full">
          <button type="button" href="#" class="inline-block w-full p-4 rounded-tr-lg hover:bg-transparent focus:outline-none dark:bg-transparent dark:hover:bg-transparent text-gray-500">Materi Kelas</button>
      </li>
    </ul> 

    <div class="mt-4 flex items-center ml-auto space-x-2 sm:space-x-3">
        <a href="{{ route('materi.create', $kelas->idkelas) }}">
          ...
          <li class="py-3 sm:py-4">
            <div class="flex items-center space-x-4">
              <div class="flex-shrink-0">
                <a href="{{ route('materi.read', $materi->idmateri) }}">
                    <img src="{{ URL('images/materi.png') }}" alt="materi" class="w-6 h-6">
                </a>
              </div>
              <style>
              ...
              <div class="flex-1 min-w-0">
                  <p class="font-medium text-gray-900 truncate dark:text-white">
                    <a href="{{ route('materi.read', $materi->idmateri) }}" class="hover-red-underline">{{ $materi->judul_materi }}</a>
                  </p>
                  <p class="text-sm text-gray-500 truncate dark:text-gray-400">
                    <a href="{{ route('materi.read', $materi->idmateri) }}" class="hover-red-underline">{{ $materi->tanggal_upload }}</a>
                  </p>
              </div>                  
              <td class="p-4 space-x-2 whitespace-nowrap">
                <a href="{{ route('materi.edit', $materi->idmateri) }}">
                  <button type="button" 
                  ...
                  <button type="button" data-modal-toggle="delete-materi-modal-{{ $materi->idmateri }}" class="inline-flex items-center px-2 py-1 text-sm font-medium text-center text-white bg-red-600 rounded-lg hover:bg-red-800 focus:ring-4 focus:ring-red-300 dark:focus:ring-red-900">
...
<!-- Hapus Tugas Modal -->
@foreach($materis as $materi)
<div class="fixed left-0 right-0 z-50 items-center justify-center hidden overflow-x-hidden overflow-y-auto top-4 md:inset-0 h-modal sm:h-full" id="delete-materi-modal-{{ $materi->idmateri }}">
  <div class="relative w-full h-full max-w-md px-4 md:h-auto">
      <!-- Modal content -->
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-800">
          <!-- Modal header -->
          <div class="flex justify-end p-2">
              <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-700 dark:hover:text-white" data-modal-toggle="delete-materi-modal-{{ $materi->idmateri }}">
          ...
          <div class="p-6 pt-0 text-center">
              ...
              <form method="POST" action="{{ route('materi.destroy', ['idkelas' => $kelas->idkelas, 'idmateri' => $materi->idmateri]) }}"  
              ...
              <a href="/viewMateri/{{ $kelas->idkelas }}" class="text-gray-900 bg-white hover:bg-gray-100 focus:ring-4 focus:ring-primary-300 border border-gray-200 font-medium inline-flex items-center rounded-lg text-base px-3 py-2.5 text-center dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700 dark:focus:ring-gray-700" data-modal-toggle="delete-class-modal">
                  Tidak hapus
              </a>
          </div>
      </div>
